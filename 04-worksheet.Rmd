...
$Y_{i,j,k}$ is 1 if player $i$ receives a red card in game $k$ officiated by referee $j$ and $X_i$ represents a vector containing player $i$'s mean skin tone rating and covariates plus the interaction term. Answer the questions below. For each question, click __Confirm__ so that you answer is properly printed in the submission. If you would like to edit your answer, you can click the __Confirm__ button again.
...
B.  League country<sup><a href="#note5">5</a></sup> (`leagueCountry`): The data set includes players playing in different leagues from four countries. It is reasonable to believe that the variable is a confounder as a league may tend to give out more red cards and have more players with darker skin tones. On the other hand, some argue that the variable is unreliable because players move between the leagues and the data set doesn't capture the movements. Consider the following options:  
...
The required libraries have been loaded and the `skintone` variable has been defined as below.
...
```{r worksheet, exercise=TRUE, exercise.lines=25, exercise.setup="worksheet-setup", exercise.eval=TRUE}
head(soccer) # the soccer data set has been loaded
```
...
1.  Print this page to PDF and save the file as _**sta490-multiverse-analysis.pdf**_. You can click the button below to open up the printer dialogue for your browser. See [here](https://www.digitaltrends.com/computing/how-to-save-a-webpage-as-a-pdf/) if you are not familiar with printing a web page to a PDF file from a browser. 
2.  Submit the pdf to Quercus. Please do **NOT** include your name in the file name or in the answers.
...
+  Scores for Question 1 and Question 2 will be used for the purpose of the `mverse` R package evaulation only.

```{r submitbutton}
shiny::actionButton(
  "printtopdf", "Print to PDF for submission", 
  shiny::icon("print"), width = "100%")
```
