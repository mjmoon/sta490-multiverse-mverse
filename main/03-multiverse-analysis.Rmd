...
ggplot(tmp, aes(y = position, x =redCards / games, colour = position_group)) +
  ...
  geom_text(aes(x = 0.07, label = paste0(position, " (n=", format(n, big.mark = ",") , ")")),
    size = 3, hjust = 1, vjust = 0, nudge_y = 0.1, nudge_x =- 0.001, show.legend = FALSE,
    data = tmp_group) +
  scale_x_continuous(position = "top", limits = c(0, 0.07)) +
...
From the table above, we observe that the results are consistent in direction and statistical significance at 95% confidence level across the multiverse of analysis paths. The results indicate that players with very dark skin tones suffer approximately 40~45% (Odd ratio 95% CIs ranging approximately within [1.05, 1.90]) higher odds of getting a red card during match compared to their colleagues with very light skin tones. 
...
<br /><footnote id="note4"><sup>4</sup><coce>filter_branch()</code> and <code>add_filter_branch()</code> are also available that multiplexes <code>filter()</code> in tidyverse.</footnote>
```
