...
We can extract the resulting estimates of $\beta$'s as shown below.

```{r glm-tut-expr-est, echo=TRUE}
summary(fit)$coefficients
```

```{r glm-tut-res}
reslog <- broom::tidy(fit, conf.int = TRUE)
```

Note that the estimate for the skin tone rating is in log-odd scale. We can compute the estimate and confidence interval in natural odd scale using `broom::tidy()`. `exponentiate = TRUE` converts the estimate to natural odd scale and `conf.int = TRUE, conf.level = .95` specifies the desired confidence interval.

```{r glm-tut-expr-tidyest, echo=TRUE}
...
In this simple demonstration, we omitted considerations for any covariates. In the following section, we demonstrate a multiverse analysis with an example for incorporating a covariate.
